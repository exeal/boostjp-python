...
Greg Burley が Unix GCC ユーザに対して以下の回答をしている。
   ...
   :program:`gdb` セッションを始めて Python による C++ ライブラリのデバッグを行うための基本的なステップを以下に示す。あなたの :abbr:`BPL` モジュール :file:`my_ext.so` を含むディレクトリで :program:`gdb` セッションを開始しなければならないことに注意していただきたい。
